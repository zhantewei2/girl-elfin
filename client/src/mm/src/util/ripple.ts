...
const isMobile=/Mobile/.test(navigator.appVersion);
  ...
  nz.runOutsideAngular(()=>{
    ...
    const listenTouch=(e:any)=>{
      ...
      if(opts.circle){
        ...
        y=e.clientY-rect.top+window.pageYOffset-opts.fontSize/2;
        x=e.clientX-rect.left-opts.fontSize/2;
      }
      appendChild(x,y);
    };
    const touchEnd=()=>{
    ...
    if(isMobile){
      node.addEventListener('touchstart',(e:any)=>listenTouch(e.touches[0]));
      if(opts.noneBg!==''&&!opts.noneBg)node.addEventListener("touchend",touchEnd);
    }else{
